...
inline bool wait_until_socket_is_ready(socket_t sock, time_t sec, 
                                       time_t usec)
{
    fd_set fdsr;
    FD_ZERO(&fdsr);
...
template <typename T>
inline bool read_and_close_socket(socket_t sock, 
...
inline void set_nonblocking(socket_t sock, bool nonblocking)
{
    ...
    fcntl(sock, F_SETFL, nonblocking ? (flags | O_NONBLOCK) : 
          (flags & (~O_NONBLOCK)));
#endif
}
...
inline const char* get_header_value(
    const Headers& headers, const char* key, size_t id = 0, 
...
inline bool read_content_with_length(Stream& strm, std::string& out, 
                                     size_t len, Progress progress)
{
    out.assign(len, 0);
    size_t r = 0;
    while (r < len){
...
bool read_content(Stream& strm, T& x, Progress progress = Progress())
{
    if (has_header(x.headers, "Content-Length")) {
        ...
        if (len == 0) {
            const auto& encoding = get_header_value(x.headers, 
                                           "Transfer-Encoding", 0, "");
    ...
    for (auto i = 0; s[i]; i++) {
        ...
        default:
            ...
            if (c >= 0x80) {
                result += '%';
                char hex[4];
                size_t len = snprintf(hex, sizeof(hex) - 1, "%02X", 
                                      c);
                assert(len == 2);
...
inline bool from_hex_to_i(const std::string& s, size_t i, 
                          size_t cnt, int& val)
{
    if (i >= s.size()) {
        return false;
...
inline void compress(std::string& content)
{
    ...
    auto ret = deflateInit2(&strm, Z_DEFAULT_COMPRESSION, 
...
inline void decompress(std::string& content)
{
    ...
    // 15 is the value of wbits, which should be at the maximum 
    //possible value to ensure
    // that any gzip stream can be decoded. The offset of 16 
...
inline std::pair<std::string, std::string> 
        make_range_header(uint64_t value, Args... args)
{
    std::string field;
        ...
        if (n >= bufsiz - 1) {
            ...
            while (n >= static_cast<int>(glowable_buf.size() - 1)) {
                ...
                n = _snprintf_s(&glowable_buf[0], glowable_buf.size(), 
                                ...
                                fmt, args...);
#else
                n = snprintf(&glowable_buf[0], glowable_buf.size() - 1, 
                             fmt, args...);
#endif
            }
...
inline int Server::bind_internal(const char* host, int port, int socket_flags)
{
    ...
    if (port == 0) {
        ...
        if (getsockname(svr_sock_, 
             reinterpret_cast<struct sockaddr *>(&address), &len) == -1) 
        {
            return -1;
        }
        if (address.ss_family == AF_INET) {
          return ntohs(reinterpret_cast<struct sockaddr_in*>
                       (&address)->sin_port);
...
inline std::shared_ptr<Response> Client::Post(
    const char* path, const Headers& headers, const std::string& body, 
    const char* content_type)
{
    Request req;
    req.method = "POST";
